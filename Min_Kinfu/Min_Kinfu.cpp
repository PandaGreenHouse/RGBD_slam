...
#pragma comment(lib, "pcl_gpu_containers_debug.lib")
#pragma comment(lib, "pcl_gpu_kinfu_debug.lib")
#pragma comment(lib, "pcl_io_ply_debug.lib")
#pragma comment(lib, "pcl_io_debug.lib")
#pragma comment(lib, "pcl_kdtree_debug.lib")
#pragma comment(lib, "pcl_kinfu_app_debug.lib")
#pragma comment(lib, "pcl_visualization_debug.lib")*/

#pragma comment(lib, "Recon_API.lib")
...
boost::shared_ptr<pcl::PolygonMesh> raw_mesh_ptr;
pcl::PolygonMesh mesh_decimated;
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
struct KinFuApp
{
  ...
  {        
	...
	if (frame_counter_ > 1) {
        ...
        {
			displayColorImage(rgb24);
			//saveImage (kinfu_.getCameraPose(), rgb24);
			//create_view_snapped(kinfu_.getCameraPose(), rgb24);
		}
	}
	frame_counter_++;
  }
  ...
  { 
	  ...
	  drawScene(kinfu_.getCameraPose(), 100, 100, 640, 480, caption, raw_mesh_ptr);
  }
  ...
  */
  ...
  writeMesh(int format) const
  {
      ...
      raw_mesh_ptr = convertToMesh(triangles_device);
	  //PolygonMesh mesh_filtered;
	  //filterMesh(*raw_mesh_ptr, mesh_filtered);
	  /*refineMesh(*raw_mesh_ptr);
	  pcl::io::saveOBJFile("raw_mesh.obj", *raw_mesh_ptr);
	  float clip_radius = 1.0f;
	  float plane_tolerance = 0.01f;
	  extractObject(*raw_mesh_ptr, clip_radius, plane_tolerance);
	  pcl::PolygonMesh mesh_decimated;
	  decimateMesh(*raw_mesh_ptr, mesh_decimated);
	  std::string raw_mesh_path, deci_mesh_path;
	  raw_mesh_path = "scene\\raw_mesh.ply";
	  deci_mesh_path = "scene\\clean.ply";
	  pcl::io::savePLYFile(raw_mesh_path, *raw_mesh_ptr);
	  pcl::io::savePLYFile(deci_mesh_path, mesh_decimated);
	  std::string scene_path = "scene";*/
	   //tex_recon(scene_path, 50);
  }
  ...
  */
  ...
  void saveImage(const Eigen::Affine3f &camPose, pcl::gpu::PtrStepSz<const PixelRGB> rgb24)
  {
	  ...
	  std::string filename_ini = "snapshots/";
	   ...
	   filename_ini = filename_ini + boost::lexical_cast<std::string> (screenshot_counter_) + file_extension_ini;
       ...
       //write ini files
	   //writeIni (filename_ini, teVecs, erreMats);
       // Save Image
  ...
  */
  ...
  /*
  created at 06/08/2017
  modified at 06/12/2017
  */
  void create_view_snapped(const Eigen::Affine3f &camPose, pcl::gpu::PtrStepSz<const PixelRGB> rgb24)
  {
	  static int view_count = 0;
	  std::string view_root_name = "scene\\views\\view_";// + boost::lexical_cast<std::string> (screenshot_counter_);
	  std::string view_name;
	  char buf[256];
	  sprintf(buf, "%0.4d%s", view_count, ".mve");
	  view_name = view_root_name + buf;
	  mkdir(view_name.c_str());
	  std::string filename_ini = view_name + "\\meta.ini";
	  //std::ofstream metaFile;
	  //metaFile.open (filename_ini.c_str());
	  float focal_length = 0.92;
	  float pixel_aspect = 0.8;
	  float pricipal_x = 1280;
	  float principal_y = 1024;
	  // Get Pose
	  ...
	  << "pixel_aspect" << " = " << 1 << endl 
	  << "principal_point" << " = " << 0.5 << " " << 0.5 << endl 
	  << "rotation = " << rotMat(0,0) <<" "<< rotMat(0,1) <<" "<< rotMat(0,2) <<" "<< rotMat(1,0) <<" "<< rotMat(1,1) <<" "<< rotMat(1,2) <<" "<< 
	  rotMat(2,0) <<" "<< rotMat(2,1) <<" "<< rotMat(2,2) << endl 
	  << "translation = " << trans(0) <<" "<< trans(1) <<" "<< trans(2) << endl 
	  << "\n[" << "view" << "]\n" << "id" << " = " << view_count << endl << "name" <<" = "<< view_count << endl;
	  out.close();
	 
	  std::string filename_image = view_name + "\\original.jpg";
	  IplImage* pImg = cvCreateImage(cvSize(rgb24.cols, rgb24.rows), 8, 3);
	  unsigned int count = 0;
	  for(int y = 0; y < rgb24.rows; y++)
	  {
		  for(int x = 0; x < rgb24.cols; x++)
		  {
			  unsigned char* dst = &CV_IMAGE_ELEM(pImg, unsigned char, y, 3*x);
			  *dst = rgb24.data[count].b;
			  dst++;
			  *dst = rgb24.data[count].g;
			  dst++;
			  *dst = rgb24.data[count].r;
			  count++;
		  }
	  }
	  cvSaveImage(filename_image.c_str(), pImg);
	  cvReleaseImage(&pImg);
	  ++view_count;
  }
  /*December 3, 2022*/
  void displayColorImage(pcl::gpu::PtrStepSz<const PixelRGB> rgb24)
  {
	  cv::Mat mat;
	  mat.create(cv::Size(rgb24.cols, rgb24.rows), CV_8UC3);
	  size_t l = 0;
	  for (size_t iy = 0; iy < mat.rows; ++iy)
	  {
		  for (size_t ix = 0; ix < mat.cols; ++ix)
		  {
			  mat.at<cv::Vec3b>(iy, ix) = cv::Vec3b(rgb24.data[l].b, rgb24.data[l].g, rgb24.data[l].r);
			  ++l;
		  }
	  }
	  cv::imshow("image", mat);
...
//  boost::shared_ptr<CameraPoseProcessor> pose_processor_;
  ...
  int nLoop_;
  int tex_map_mode_;
  float clip_width_, clip_height_, clip_depth_;
  //openni2
...
void filterMesh(PolygonMesh &input_mesh, PolygonMesh &out_mesh)
{
	...
	vector<int> vt_labels;
	vt_labels.resize(input_cloud->size());
	cout << "start refining mesh\n" << flush;
	for(int i=0; i < vt_labels.size(); i++)
	{
		vt_labels[i] = 1;
	}
	out_mesh.header = input_mesh.header;
	vector<int> vt_indices;
	vt_indices.resize(input_cloud->size());
	vt_indices[0] = 0;
	int count = 0;
	for(int j=0; j < input_cloud->size(); j++)
	{
		...
		vt_indices[j] = count;
		for(int i=j+1; i < input_cloud->size(); i++)
		{
	...
	{
		unsigned int v0[3], v1[3];
		v0[0] = input_mesh.polygons[t].vertices[0];
		v0[1] = input_mesh.polygons[t].vertices[1];
		v0[2] = input_mesh.polygons[t].vertices[2];
		v1[0] = vt_indices[v0[0]];
		v1[1] = vt_indices[v0[1]];
		v1[2] = vt_indices[v0[2]];
		if(v1[0]==v1[1] || v1[0]==v1[2])
			continue;
		if(v1[1]==v1[2])
			continue;
		pcl::Vertices face;
		face.vertices.push_back(v1[0]);
		face.vertices.push_back(v1[1]);
		face.vertices.push_back(v1[2]);
		out_mesh.polygons.push_back(face);
	}
	cout << "complete refining mesh" << flush;
...
{  
  ...
  //03/30/2017
  unsigned int keyframe_count = 1200;
  if (pc::parse_argument (argc, argv, "-max-frames", keyframe_count) > 0)
	  app.setLimitCount(keyframe_count);
  //03/14/2017
  ...
  int deci_level = 4;
  pc::parse_argument (argc, argv, "-deci-level", deci_level);
  app.nLoop_ = deci_level;
  int tex_map_mode = 0;
  pc::parse_argument (argc, argv, "-mode", tex_map_mode);
  app.tex_map_mode_ = tex_map_mode;
  float clip_width = 1.5f;
  pc::parse_argument (argc, argv, "-clip-width", clip_width);
  app.clip_width_ = clip_width;
  float clip_height = 1.5f;
  pc::parse_argument (argc, argv, "-clip-height", clip_height);
  app.clip_height_ = clip_height;
  float clip_depth = 1.5f;
  pc::parse_argument (argc, argv, "-clip-depth", clip_depth);
  app.clip_depth_ = clip_depth;
  // executing
  try { app.startMainLoop (triggered_capture); }
  catch (const pcl::PCLException& /*e*/) { cout << "PCLException" << endl; }
  catch (const std::bad_alloc& /*e*/) { cout << "Bad alloc" << endl; }
  catch (const std::exception& /*e*/) { cout << "Exception" << endl; }
  /*std::string in_scene("scene::original");
  std::string tex_mesh("mvs_textured");
  createTex_Mesh("scene", in_scene, "scene\\surface.ply", tex_mesh);*/
#ifdef HAVE_OPENCV
